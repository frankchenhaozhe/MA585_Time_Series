...
fit0 <- armasubsets(y = train, nar = 14, nma = 14)
plot(fit0)
# choose the 5th model

fit1 <- arima(train, order=c(14,0,12), fixed = C(NA,NA, rep(0,9), rep(NA,6), rep(0,8), NA, NA))
forecast(fit1, h = length(test))
...
`RMSE (Root Mean Squared Error)` <- c(NA,NA,NA)
`MAPE (Mean Average Percent Error)` <- c(NA,NA,NA)
